...
GETSETPHYSETTINGS icsneoReadWritePHYSettings;

GETDEVICESETTINGS icsneoGetDeviceSettings;
SETDEVICESETTINGS icsneoSetDeviceSettings;
...
ENABLEDOIPLINE icsneoEnableDOIPLine;
...
bool LoadDLLAPI(HINSTANCE &hAPIDLL)
{
	...
	icsneoFindDevices = (FINDDEVICES)GetProcAddress(hAPIDLL,						"icsneoFindDevices"); 
...
////	icsneoEnableNetworkCom = (ENABLENETWORKCOM) GetProcAddress(hAPIDLL,             "icsneoEnableNetworkCom");
	...
	icsneoGetDeviceSettings = (GETDEVICESETTINGS) GetProcAddress(hAPIDLL,            "icsneoGetDeviceSettings");
	icsneoSetDeviceSettings = (SETDEVICESETTINGS) GetProcAddress(hAPIDLL,            "icsneoSetDeviceSettings");
	...
	icsneoReadWritePHYSettings = (GETSETPHYSETTINGS) GetProcAddress(hAPIDLL, "icsneoReadWritePHYSettings");
	...
	icsneoEnableDOIPLine = (ENABLEDOIPLINE) GetProcAddress(hAPIDLL,     "icsneoEnableDOIPLine");
	icsneoEnableNetworkComEx = (ENABLENETWORKCOMEX) GetProcAddress(hAPIDLL,     "icsneoEnableNetworkComEx");

    if(!icsneoFindNeoDevices || !icsneoOpenNeoDevice || !icsneoClosePort || !icsneoFreeObject || !icsneoFindDevices ||
	   ...
	   !icsneoSetBitRate || !icsneoSetFDBitRate || !icsneoGetDeviceParameters || !icsneoSerialNumberToString ||
	   ...
	   !icsneoScriptWriteAppSignal || !icsneoGetDLLVersion || !icsneoGetDeviceSettings || !icsneoGetDeviceSettings || 
	   !icsneoReadWritePHYSettings || !icsneoEnableDOIPLine)
	{
		FreeLibrary(hAPIDLL);
		return false;
	}
